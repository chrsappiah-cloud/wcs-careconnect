...
async function testRxNorm() {
  ...
  if (search.ok && search.data?.drugGroup?.conceptGroup) {
    const groups = search.data.drugGroup.conceptGroup.filter(g => g.conceptProperties);
    report("RxNorm: searchDrugs(metformin)", "PASS", `${groups.length} concept groups`);
  } else {
    report("RxNorm: searchDrugs(metformin)", "FAIL", search.error || `HTTP ${search.status}`);
  }
  ...
  if (detail.ok && detail.data?.allRelatedGroup) {
    ...
    report("RxNorm: getDrugByRxcui(861004)", "FAIL", detail.error || `HTTP ${detail.status}`);
  }
  ...
  if (labelInteract.ok && labelInteract.data?.results?.length > 0) {
    const hasSection = labelInteract.data.results[0].drug_interactions?.length > 0;
    report("OpenFDA: checkDrugInteractions(label)", hasSection ? "PASS" : "WARN", hasSection ? "interaction text found" : "no interaction section");
  } else {
    report("OpenFDA: checkDrugInteractions(label)", "FAIL", labelInteract.error || `HTTP ${labelInteract.status}`);
  }
  ...
  if (spell.ok && spell.data?.suggestionGroup?.suggestionList?.suggestion?.length > 0) {
    report("RxNorm: suggestDrugSpelling", "PASS", `${spell.data.suggestionGroup.suggestionList.suggestion.length} suggestions`);
  } else {
    report("RxNorm: suggestDrugSpelling", spell.ok ? "WARN" : "FAIL", spell.error || "no suggestions");
  }
}
...
async function testOpenFDA() {
  ...
  if (events.ok && events.data?.results?.length > 0) {
    report("OpenFDA: searchAdverseEvents(metformin)", "PASS", `${events.data.results.length} events`);
  } else {
    report("OpenFDA: searchAdverseEvents(metformin)", "FAIL", events.error || `HTTP ${events.status}`);
  }
  ...
  if (top.ok && top.data?.results?.length > 0) {
    report("OpenFDA: topAdverseReactions(metformin)", "PASS", `top reaction: ${top.data.results[0].term}`);
  } else {
    report("OpenFDA: topAdverseReactions(metformin)", "FAIL", top.error || `HTTP ${top.status}`);
  }
}
...
async function testICD11() {
  ...
  if (search.ok && search.data?.destinationEntities?.length > 0) {
    report("ICD-11: searchConditions(diabetes)", "PASS", `${search.data.destinationEntities.length} results`);
  } else if (search.status === 401 || search.status === 403) {
    report("ICD-11: searchConditions(diabetes)", "WARN", `HTTP ${search.status} (auth required — will fallback to SNOMED)`);
  } else {
    report("ICD-11: searchConditions(diabetes)", "FAIL", search.error || `HTTP ${search.status}`);
  }
}
...
async function testHAPIFHIR() {
  ...
  if (patients.ok && patients.data?.entry?.length > 0) {
    report("FHIR: searchPatients(smith)", "PASS", `${patients.data.entry.length} patients`);
  } else {
    report("FHIR: searchPatients(smith)", patients.status === 0 ? "FAIL" : "WARN", patients.error || `HTTP ${patients.status}`);
  }
  ...
  if (meds.ok) {
    ...
    report("FHIR: searchMedications(aspirin)", count > 0 ? "PASS" : "WARN", `${count} results`);
  } else {
    report("FHIR: searchMedications(aspirin)", "FAIL", meds.error || `HTTP ${meds.status}`);
  }
  ...
  if (obs.ok) {
    ...
    report("FHIR: searchObservations(BP)", count > 0 ? "PASS" : "WARN", `${count} observations`);
  } else {
    report("FHIR: searchObservations(BP)", "FAIL", obs.error || `HTTP ${obs.status}`);
  }
}
...
async function testOntoserver() {
  ...
  if (lookup.ok && lookup.data?.parameter?.length > 0) {
    const display = lookup.data.parameter.find(p => p.name === "display")?.valueString;
    report("Ontoserver: lookupSNOMED(73211009)", "PASS", display || "resolved");
  } else {
    report("Ontoserver: lookupSNOMED(73211009)", "FAIL", lookup.error || `HTTP ${lookup.status}`);
  }
  ...
  if (findings.ok && findings.data?.expansion?.contains?.length > 0) {
    report("Ontoserver: searchSNOMEDFindings(hypertension)", "PASS", `${findings.data.expansion.contains.length} findings`);
  } else {
    report("Ontoserver: searchSNOMEDFindings(hypertension)", "FAIL", findings.error || `HTTP ${findings.status}`);
  }
  ...
  if (amt.ok && amt.data?.expansion?.contains?.length > 0) {
    report("Ontoserver: searchAMTMedications(paracetamol)", "PASS", `${amt.data.expansion.contains.length} medications`);
  } else {
    ...
    if (intl.ok && intl.data?.expansion?.contains?.length > 0) {
      report("Ontoserver: searchAMTMedications(paracetamol)", "PASS", `${intl.data.expansion.contains.length} meds (intl fallback)`);
    } else {
      report("Ontoserver: searchAMTMedications(paracetamol)", "WARN", "AMT + intl both empty; dev server may lack data");
    }
  }
}
...
async function testInternalAPI() {
  ...
  const base = process.env.EXPO_PUBLIC_CREATE_API_URL || process.env.EXPO_PUBLIC_BASE_URL || "https://createanything.com";

  const endpoints = [
    { name: "GET /api/residents", url: `${base}/api/residents` },
    { name: "GET /api/alerts?status=open", url: `${base}/api/alerts?status=open` },
  ...
  for (const ep of endpoints) {
    ...
    if (res.ok && Array.isArray(res.data)) {
      ...
      report(`Internal: ${ep.name}`, "WARN", `HTTP ${res.status}, non-array response`);
    } else if (res.status === 401 || res.status === 403) {
      report(`Internal: ${ep.name}`, "WARN", `HTTP ${res.status} (auth required — expected without JWT)`);
    } else if (res.status === 429) {
      report(`Internal: ${ep.name}`, "WARN", `HTTP ${res.status} (rate-limited — expected without JWT)`);
    } else {
...
async function main() {
  ...
  console.log("\n══════════════════════════════════════════════════════════");
  console.log(`  SUMMARY: ${pass} passed, ${warn} warnings, ${fail} failed  (${results.length} total)`);
  console.log("══════════════════════════════════════════════════════════\n");
