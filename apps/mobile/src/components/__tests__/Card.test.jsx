...
describe('Card', () => {
  ...
  it('applies default padding of 20', () => {
    const { toJSON } = render(<Card><Text>Padded</Text></Card>);
    const root = toJSON();
  ...
  it('removes padding when noPadding is true', () => {
    const { toJSON } = render(<Card noPadding><Text>NP</Text></Card>);
    const root = toJSON();
  ...
  it('applies elevated variant shadow', () => {
    const { toJSON } = render(<Card variant="elevated"><Text>Elevated</Text></Card>);
    const root = toJSON();
    const flatStyle = Object.assign({}, ...root.props.style.filter(Boolean));
  ...
  it('applies custom style', () => {
    const { toJSON } = render(<Card style={{ marginTop: 99 }}><Text>Custom</Text></Card>);
    const root = toJSON();
    const flatStyle = Object.assign({}, ...root.props.style.filter(Boolean));
  ...
  it('uses surface background and surface border', () => {
    const { toJSON } = render(<Card><Text>BG</Text></Card>);
    const root = toJSON();
